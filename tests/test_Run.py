...
class TestRun(unittest.TestCase):
    ...
    mock_run_step_list_data = MagicMock(
        object = "list",
        data = [
            mock_run_step_data,
            mock_run_step_data
        ],
        first_id= "test_run_step_id",
        last_id= "test_run_step_id",
        has_more= False
    )
    ...
    def setUp(self, mock_openai):
        ...
        mock_openai.return_value.beta.threads.runs.steps.retrieve.return_value = self.mock_run_step_data
        mock_openai.return_value.beta.threads.runs.steps.list.return_value = self.mock_run_step_list_data
    ...
    # Test for retrieve_run method
    @patch('GPTManager.Client.OpenAI')
    def test_retrieve_run_step(self, mock_openai):
        run_step = self._run.retrieve_run_step('test_run_step_id')
        self.assertEqual(run_step.object, 'thread.run.step')
    ...
    @patch('GPTManager.Client.OpenAI')
    def test_list_run_steps(self, mock_openai):
        run_steps = self._run.list_run_steps()

        # Assert that the method returns a list of Run objects
        self.assertIsInstance(run_steps, list)
        self.assertEqual(len(run_steps), 2)
        self.assertIsInstance(run_steps[0], RunStep)
        self.assertIsInstance(run_steps[1], RunStep)

        # Assert that each Run object has the correct attributes set
        self.assertEqual(run_steps[0].id, "test_run_step_id")
        self.assertEqual(run_steps[0].object, 'thread.run.step')

        self.assertEqual(run_steps[1].id, "test_run_step_id")
        self.assertEqual(run_steps[0].object, 'thread.run.step')
